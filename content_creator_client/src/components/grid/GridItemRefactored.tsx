...
import { ImageCardComponent, BadgeComponent, AbsoluteTopRightComponent } from '../ui'
...
const GridItemRefactored: React.FC<GridItemProps> = ({
  title,
  summary,
  imageUrl,
  imageAlt,
  ...
  return (
    ...
    >
      ...
      {children && (
        <div id={`grid-item-${prefixId ?? 'item'}-children`}>
          {children}
        </div>
      )}
      ...
      {typeof summary !== 'string' && summary && (
        <div id={`grid-item-${prefixId ?? 'item'}-summary-content`}>
          {summary}
        </div>
      )}
    </ImageCardComponent>
...
export default GridItemRefactored
